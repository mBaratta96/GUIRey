...
def selectFiles():
    return filedialog.askopenfilenames(initialdir = os.getcwd(),title = "Select images",filetypes = ( ("png files",".png"),("jpeg files","*.jpg")))
...
class MainMenu(object):
    ...
    def __init__(self, root):
        ...
        self.cropTemplateButton = tk.Button(self.menu_frame, text="Crop Image", command=self.openCropRegion).pack(
            fill='both')
        self.analizeScoreButton =  tk.Button(self.menu_frame, text="Compute Similarity Measures", command=self.openSimMeasures).pack(
            fill='both')
        self.findTempButton = tk.Button(self.menu_frame, text="Find Cropped Image",
                                            command=self.openTempMatching).pack(
            fill='both')
